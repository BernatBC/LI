...
politician(N,MaxKm,Trip):-     path(N,MaxKm, 1,0,Trip).
...
path( NumCitiesRemainingToBeVisited, RemainingKm, CurrentCity, CitiesAlreadyVisited, [CurrentCity1|Path] ):-
    ...
    Cities is NumCitiesRemainingToBeVisited - 1,
    Km is RemainingKm - K,
    not(member(CurrentCity1, CitiesAlreadyVisited)),
    path(Cities, Km, CurrentCity1, [CurrentCity1|CitiesAlreadyVisited], Path).
