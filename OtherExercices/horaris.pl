...
eachCourseExactlyOneRoom:- courseRooms(C,LR), findall(cr(C,R), (member(R,LR)), Lits), exactly(1,Lits), findall(cr(C,R), room(R),Lits2), exactly(1,Lits2), fail.
eachCourseExactlyOneRoom.
...
eachCourseRightNumberOfDays:- course(C), findall(cd(C,D),day(D),Lits), courseHours(C,Hs), exactly(Hs,Lits), fail.
eachCourseRightNumberOfDays.

noOverlapCoursesSameYear:- year(Y), day(D), hour(H), findall(cdh(C,D,H), courseYear(C,Y), Lits), atMost(1, Lits), fail.
noOverlapCoursesSameYear.

noOverlapProfs:- professor(P), day(D), hour(H), course(C1), course(C2), C1 \= C2, writeClause([ -cdh(C1,D,H), -cdh(C2,D,H), -cp(C1,P), -cp(C2,P) ]), fail. 
noOverlapProfs.

noOverlapRooms:- room(R), day(D), hour(H), course(C1), course(C2), C1 \= C2, writeClause([ -cdh(C1,D,H), -cdh(C2,D,H), -cr(C1,R), -cr(C2,R) ]), fail.
noOverlapRooms.

relateVarsCPwithUsedProf:- course(C), professor(P), writeClause([-cp(C,P),usedProf(P)]), fail.
relateVarsCPwithUsedProf.
