...
eachEexactlyOneD:- event(E), findall(ed(E,D), day(D), Events), exactly(1, Events), fail.
eachEexactlyOneD.

eachEremoveNonDays:- event(E), findall(ed(E,D), day(D), Events), member(Ev, Events), Ev=ed(E,Dy), eventDays(E,Days), not(member(Dy,Days)), exactly(0, [Ev]), fail.
eachEremoveNonDays.

eachEexactlyOneM:- event(E),  findall(em(E,M), moderator(M), Events), exactly(1, Events), fail.
eachEexactlyOneM.

eachEremoveNonModerators:- event(E), findall(em(E,M), moderator(M), Events), member(Ev, Events), Ev=em(E,Mo), eventModerators(E,Mods), not(member(Mo,Mods)), exactly(0, [Ev]), fail.
eachEremoveNonModerators.

eachDmaxE:- day(D), findall(ed(E,D), event(E), Events), maxEventsPerDay(N), atMost(N,Events), fail.
eachDmaxE.

eachMmaxD:- moderator(M), findall(md(M,D), day(D), Days), maxDaysPerModerator(N),atMost(N, Days), fail.
eachMmaxD.

linkVars:- event(E), day(D), moderator(M), negate(ed(E,D), V1), negate(em(E,M), V2), writeOneClause([V1,V2,md(M,D)]), fail.
linkVars.
...
main:-  initClauseGeneration,
        ...
        shell('./kissat-rel-3.0.0/build/kissat -v infile.cnf > model', Result),  % if sat: Result=10; if unsat: Result=20.
        treatResult(Result),!.
