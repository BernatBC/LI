...
incompatibleWorkers(W1,W2):- W1 \= W2, workerScore(W1, S1), workerScore(W2,S2), maxScore(M), S is S1 + S2, S > M.
...
eachWexactlyOneT:- worker(W), findall(wt(W,T), team(T), Teams), exactly(1,Teams), fail.
eachWexactlyOneT.
