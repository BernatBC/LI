...
dayBus(D,B):-               day(D), bus(B).
city(C):-                   traject(C,_,_,_); traject(_,C,_,_).
...
writeClauses:-  
    ...
    eachRouteAtLeastNDays,
    eachRouteAtMostOnceADay,
...
eachRouteAtLeastNDays:- trip(C1-C2), findall(bdcc(B,D,C1,C2),dayBus(D,B), L), frequency(C1-C2,F), atLeast(F, L), fail.
eachRouteAtLeastNDays.

eachRouteAtMostOnceADay:- trip(C1-C2), day(D), findall(bdcc(B,D,C1,C2), bus(B), L), atMost(1, L), fail.
eachRouteAtMostOnceADay.
