...
differentTaskOnConsecutive:- gangster(G), hour(H1), hour(H2), H1 is H2 - 1, task(T1), task(T2), T1 \= T2, atMost(1, [does(G,T1,H1), does(G,T2,H2)]), fail.
differentTaskOnConsecutive.

notAvailableOnHour:- gangster(G), task(T), blocked(G, H), exactly(0, [does(G,T,H)]), fail.
notAvailableOnHour.

assignTasks:- hour(H), task(T), needed(T,H,N), findall(does(G,T,H),gangster(G), Lits), exactly(N, Lits), fail.
assignTasks.

maxHours(MaxConsecutiveHours):- gangster(G), task(T), hour(H1), hour(H2), H2 is H1 + MaxConsecutiveHours, findall(does(G, T, H), between(H1, H2, H), Lits), negateAll(Lits, NLits), atLeast(1, NLits), fail.
maxHours(_).
...
costOfThisSolution(M,Cost):- between(0, 71, Num), Cost is 72 - Num, gangster(G), findall(H, member(does(G,_,H),M),Hours), append([_,List,_],Hours), length(List, Cost), consecutive(List), !.
