...
satVariable( villageHasTower(V)):- village(V).
...
writeClauses(MaxNumTowers):-
    noTowersOutside,
    villageMaxOneTower,
    significantVillagesExactlyOneTower,
    putMaxTowers(MaxNumTowers),
    watchedAtLeastOnce,
    linkTowers,
    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

noTowersOutside:- findall(towerPos(I,J), (position(I,J), not(posVillage(_,I,J))), Lits), exactly(0,Lits).

villageMaxOneTower:- village(V), findall(towerPos(I,J), posVillage(V,I,J), Lits), atMost(1, Lits), fail.
villageMaxOneTower.

significantVillagesExactlyOneTower:- significantVillage(V) ,findall(towerPos(I,J), posVillage(V,I,J), Lits), exactly(1, Lits), fail.
significantVillagesExactlyOneTower.

watchedAtLeastOnce:- village(V), findall(towerPos(I,J), posWatchesVillage(V,I,J), Lits), atLeast(1, Lits),fail.
watchedAtLeastOnce.

putMaxTowers(MaxNumTowers):- findall(villageHasTower(V), village(V), Lits), atMost(MaxNumTowers, Lits).

linkTowers:- posVillage(V,I,J), writeOneClause([-towerPos(I,J), villageHasTower(V)]), fail.
linkTowers.
...
costOfThisSolution(M,CostR):- length(M,Cost), CostR is Cost/2.
